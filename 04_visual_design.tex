...
To create a new chart, the user can use the ``plus'' button at the end of the navigation bar.
When the user clicks the button, a new tab is created and displayed (\cref{fig:home}):
...
\paragraph{Position}
Position is used to encode the value of $2$ indicators:
...
\paragraph{Size}
Size is used to encode the third indicator, in our case the population size.
...
On the other hand, it is difficult to understand the real absolute values of the populations.

\paragraph{Color}
\label{paragraph:bubbles-color}
Color can be used to encode any of the available dimensions of the dataset.
...
All labels are gray (a neutral color) not to attract the attention;
...
\begin{itemize}
	...
	\item There is no legend for the size of the areas.
\end{itemize}
...
This information is only encoded in text labels inside each bar, which are not always easy to read.
...
The labels with the name of countries are of the same color of the line that corresponds to them.
This is a good design choice since it uses color to make the association line-label easier to read. 
...
We will first discuss the common ones, then we will focus on specific problems.
...
Unfortunately, the labels use a color very similar to the one of the bars, which makes them difficult to read.
